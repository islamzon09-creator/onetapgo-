...
function render(){
 ...
 document.getElementById('gallery').innerHTML=photos.map((p,i)=>`<button class="memory" style="${p.src?'':`background:${fallback[i%fallback.length]}`}">${p.src?`<img src="${p.src}" alt="${p.caption||''}">`:`<span style="inset:0;display:grid;place-items:center;text-align:center;font:italic 22px 'Cormorant Garamond'">Our memory ♡</span>`}<span>${p.caption||''}</span></button>`).join('');
